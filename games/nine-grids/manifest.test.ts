// games/nine-grids/manifest.test.ts — G-12 (platform-corrections.md M2 entry checklist,
...
describe("nine-grids manifest", () => {
  ...
  it("id matches engine.meta.id (plan §5.2's own contract)", () => {
    expect(manifest.id).toBe(nineGrids.meta.id);
  });

  it("players.min/max matches engine.meta.minPlayers/maxPlayers", () => {
    expect(manifest.players.min).toBe(nineGrids.meta.minPlayers);
    expect(manifest.players.max).toBe(nineGrids.meta.maxPlayers);
  });
});
