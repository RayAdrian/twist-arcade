# `tilt` — build checklist

Stamped by `pnpm new-game tilt`. Work through this in order — it doubles as the
...
1. **`Move` needs an explicit index signature.** A `Move` type used as the engine's `M`
   ...
   `TiltMove` in `engine.ts` for the pattern every fixture in this repo uses).
...
- [ ] `engine.ts` — replace `computeStatus`'s TODO with your real win/draw rules.
      `pnpm --filter @twist-arcade/tilt test` should turn the "termination" failure
      green (see `engine.ts`'s own module doc for exactly what should change).
- [ ] `engineContract` green (`pnpm --filter @twist-arcade/tilt test`) — every other
...
- [ ] `pnpm harness solve tilt` if your state space is under ~10^7 (see
...
- [ ] `pnpm harness suite tilt --suite ci` green.
- [ ] `pnpm harness suite tilt --suite design` reviewed (Fable stage-3/6, tighter bands
...
- [ ] **Gates green, THEN register** (platform-corrections.md C15/C28): this game was
      scaffolded UNREGISTERED on purpose — gate-before-UI (C16) means `/play/tilt` must
      not be routable until `pnpm harness:ci-gates -- --game tilt` passes. Once it does,
      run `pnpm new-game tilt --register` (same id — this re-run inserts the
