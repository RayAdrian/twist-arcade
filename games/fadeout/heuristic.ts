...
export function fadeoutHeuristic(
  ...
  for (const line of LINES) {
    ...
    if (ownPresent && !oppPresent) score += ownWeight;
    if (oppPresent && !ownPresent) score -= oppWeight;
  }

  const positionValue = (cell: number): number => {
    const center = Math.floor((config.boardSize * config.boardSize) / 2);
    if (cell === center) return 3;
  ...
  for (const cell of ownQueue) score += positionValue(cell) * 0.1;
  for (const cell of oppQueue) score -= positionValue(cell) * 0.1;
