...
| 56421–56429 (team 21) | duel-draft | CLAIMED — verified free with `lsof -nP -iTCP:<port> -sTCP:LISTEN` before claiming (all 9 ports silent) |
| 56521–56529 (team 22) | mirrorna | CLAIMED — verified free with `lsof -nP -iTCP:<port> -sTCP:LISTEN` AND `docker ps` (63 unrelated containers running on this host; none publish a port in this range) before claiming, per C62's "lsof alone is not sufficient" finding |
